...
var logger = log4js.getLogger('Jeadigital-Customer-Loyalty-App:[utils.js]');
//let logger = utils.getLogger('invoke-chaincode.js');
...
let invokeChaincode = async function (peerNames, channelName, chaincodeName, fcn, args, username, org_name) {
	...
	try {
		...
		for (const i in proposalResponses) {
			if (proposalResponses[i] instanceof Error) {
				all_good = false;
				error_message = util.format(proposalResponses[i].toString());
	...
	if (!error_message) {
		let message = util.format('Successfully invoked the chaincode');
		logger.info(message);
		const response = {
			success: true,
			status: 201,
		...
		let message = util.format('Failed to invoke chaincode.');
		logger.error(message);
		const response = {
			success: false,
			status: 400,
