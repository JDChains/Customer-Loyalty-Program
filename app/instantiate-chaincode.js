...
let logger = utils.getLogger('instantiate-chaincode.js');

// Instantiate a chaincode
let instantiateChaincode = async function (peers, channelName, chaincodeName, chaincodeVersion, functionName, chaincodeType, args, username, org_name, isUpgrade) {
	...
	try {
		...
		let request = {
			...
			'endorsement-policy': {
				identities: [
					{ role: { name: 'member', mspId: 'CustomerMSP' } },
					{ role: { name: 'member', mspId: 'CompanyMSP' } }
		...
		if (isUpgrade && isUpgrade == true) {
			results = await channel.sendUpgradeProposal(request,240000); //instantiate takes much longer
		}

		results = await channel.sendInstantiateProposal(request, 240000); //instantiate takes much longer
